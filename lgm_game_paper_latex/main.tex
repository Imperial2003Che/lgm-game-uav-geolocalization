...
Fig.~\ref{fig:framework} shows the proposed pipeline. First, a visual encoder extracts multi-scale UAV and satellite tokens. Second, a vision-language model generates semantic descriptions and stable semantic anchors. Third, vector maps are converted into topology tokens. Fourth, a language-map-guided geometric sparse Transformer performs cross-view interaction. Finally, a Sinkhorn-MaxClique module refines local matching and filters candidate regions by graph consistency.

\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{figures/framework_overview.pdf}
\caption{Overview of the proposed LGM-GAME framework. Visual evidence from UAV and satellite views is combined with VLM-derived semantic anchors, style prompts, and vector-map topology before Sinkhorn matching and graph-consistency verification.}
\label{fig:framework}
\end{figure*}
...
\begin{figure*}[!t]
\centering
\includegraphics[width=0.92\textwidth]{figures/attention_module.pdf}
\caption{Text-style-map guided attention logit. The module rewards appearance, geometric plausibility, stable semantic anchors, and vector-map compatibility, while suppressing style-only shortcuts.}
...
In the current local run, the complete datasets under \texttt{C:\textbackslash Programs\textbackslash IMTMN\textbackslash datasets} are used for checkpoint evaluation. University-1652 provides UAV, satellite, and street-view query/gallery splits, while SUES-200 provides multi-altitude UAV imagery and satellite references for 200 locations.
...
\subsection{Real-Data Checkpoint Evaluation}
To replace the earlier synthetic-only prototype validation, we run a full-query retrieval evaluation on the local University-1652 and SUES-200 datasets. The experiment uses the available trained local checkpoint as the real-image feature extractor and evaluates every query in each selected split. Therefore, these numbers should be interpreted as a real-data diagnostic checkpoint for the current workspace, not as a published-method comparison or the final LGM-GAME public benchmark result. The complete LGM-GAME submission should still add method-specific training, ablations, competing baselines, and repeated-run statistics.

\input{tables/real_retrieval_table.tex}

\begin{figure*}[!t]
\centering
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{figures/real_retrieval_metrics.pdf}
\end{minipage}\hfill
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{figures/real_cmc_curves.pdf}
\end{minipage}
\caption{Real-data retrieval results on local University-1652 and SUES-200 splits. Left: Recall@1, Recall@5, Recall@10, and mAP for each task. Right: CMC curves showing the retrieval hit rate as the rank threshold increases.}
\label{fig:real_retrieval_results}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=0.95\textwidth]{figures/real_retrieval_examples.pdf}
\caption{Qualitative top-5 retrieval examples from the real local datasets. Each row shows the query image followed by retrieved satellite candidates; green borders indicate correct class matches and red borders indicate mismatches.}
\label{fig:real_retrieval_examples}
\end{figure*}

\subsection{Published Baselines and Executable Training}
For external comparison, Table~\ref{tab:publication_baselines} reports published University-1652 results from public cross-view geo-localization methods, including LPN \cite{lpn2022} and CA-HRS \cite{cahrs2022}. These numbers are reported from the corresponding papers and are not produced by the local diagnostic checkpoint. We further train an executable LGM-GAME-lite alignment head on the full University-1652 training split. This head is trained on cached real-image features with cross-view class InfoNCE supervision and optional feature-style consistency. Because the local datasets do not yet include VLM captions or vector-map alignments, this experiment should be read as an available-data implementation of the content-alignment and graph-consistency parts of LGM-GAME. The full text-anchor and vector-map ablations should be rerun after those annotations are generated.

\input{tables/publication_training_protocol_table.tex}
\input{tables/publication_baseline_table.tex}

\subsection{Ablation, Repeatability, and Efficiency}
Table~\ref{tab:publication_ablation} reports the available-data ablation. The trained content head improves macro Recall@1 and mAP over the raw feature cosine baseline, while the style-invariant head is more conservative because no real weather or season labels are available for supervision. Table~\ref{tab:publication_repeated} reports three repeated training seeds, and Table~\ref{tab:publication_efficiency} summarizes the CPU ranking cost, feature memory, and extra parameter count.

\input{tables/publication_ablation_table.tex}
\input{tables/publication_repeated_stats_table.tex}
\input{tables/publication_efficiency_table.tex}

\begin{figure*}[!t]
\centering
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{figures/publication_ablation_bars.pdf}
\end{minipage}\hfill
\begin{minipage}{0.49\textwidth}
\centering
\includegraphics[width=\linewidth]{figures/publication_robustness_groups.pdf}
\end{minipage}
\caption{Compact publication figures for the executable experiment suite. Left: macro ablation results on the six full-query retrieval tasks. Right: SUES-200 altitude grouping and feature-level stress robustness.}
\label{fig:publication_ablation_robustness}
\end{figure*}

\subsection{Robustness Analysis}
The robustness experiments should evaluate environmental changes, partial overlap, repetitive structures, low-texture regions, and large-scale candidate databases. In the current local data, SUES-200 provides real altitude groups, while explicit weather and season labels are unavailable. Therefore, Table~\ref{tab:publication_robustness} reports SUES-200 altitude groups and feature-level stress tests as an interim robustness analysis. The final journal submission should replace feature-level stress with image-level weather, illumination, season, and sensor-style splits when metadata or VLM-assisted labels are available.

\input{tables/publication_robustness_table.tex}

\subsection{Qualitative and Figure Design}
Recent TGRS papers typically use more than quantitative tables; they show why the method works. Fig.~\ref{fig:real_retrieval_examples} provides real retrieval examples from the current local datasets. The final submission should further include the following compact visual evidence:
\begin{itemize}
